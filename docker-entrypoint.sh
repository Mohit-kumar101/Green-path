...
resolve_mongo_uri() {
  for key in SPRING_DATA_MONGODB_URI MONGODB_URI MONGO_URI MONGO_URL DATABASE_URL; do
...
if [ -n "$mongo_uri" ]; then
  ...
  exec java $JAVA_OPTS -Dspring.data.mongodb.uri="$mongo_uri" -jar /app/app.jar
fi
